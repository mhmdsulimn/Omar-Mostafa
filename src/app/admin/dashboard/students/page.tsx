      ...
      <Card className="rounded-2xl overflow-hidden border-none shadow-xl bg-card/50 backdrop-blur-sm">
        ...
        <CardContent className="p-0">
          {filteredUsers.length === 0 ? <div className="text-center py-20 opacity-50 font-bold">لم يتم العثور على نتائج.</div> : (
            <Table><TableHeader className="bg-muted/30"><TableRow><TableHead className="text-right font-bold p-4">بيانات الطالب</TableHead><TableHead className="text-center font-bold p-4">الإجراءات</TableHead></TableRow></TableHeader>
            <TableBody>{filteredUsers?.map((user) => (<UserRow key={user.id} user={user} />))}</TableBody></Table>
          )}
        </CardContent>
      </Card>
    </div>
